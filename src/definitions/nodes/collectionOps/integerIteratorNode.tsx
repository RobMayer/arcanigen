...
const def = signature({
    in: {
        ...
        stops: $.arrayOf("stop:integer"),
        "stop_*": "stop:integer",
    },
    out: { sequencedOutput: "integer", sampledOutput: "integer", stopCount: "integer" },
});

export type IntegerIteratorDefinition = SignatureBuilder.DefinitionFrom<
    typeof def,
    {
        label: string;
...
const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<IntegerIteratorDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    ...
    const handleAddStop = useCallback(() => {
        ...
        alterNode(node.id, (n) => ({
            ...
            payload: {
                ...n.payload,
                stops: [...(n.payload as IntegerIteratorDefinition["payload"]).stops, { socket, value: "5", position: "50", enabled: true }],
            },
        }));
    }, [alterNode, node.id]);
        ...
        (socket: string) => {
            ...
            alterNode(node.id, (n) => {
                ...
                return {
                    ...
                    payload: {
                        ...n.payload,
                        stops: (n.payload as IntegerIteratorDefinition["payload"]).stops.filter((s) => s.socket !== socket),
                    },
                };
            });
        },
...
const StopEntry = ({
    ...
    node: NodeDefinitions.NodeFor<IntegerIteratorDefinition>;
    index: number;
...
const resolveStops = (node: NodeDefinitions.NodeFor<IntegerIteratorDefinition>, context: Resolver.Context): { value: number; position: number; enabled: boolean }[] => {
...
const dependsOn = (
    node: NodeDefinitions.NodeFor<IntegerIteratorDefinition>,
    outSocket: keyof IntegerIteratorDefinition["outputs"],
    _deps: AllDeps,
): (keyof IntegerIteratorDefinition["inputs"])[] => {
...
const evaluate = (node: NodeDefinitions.NodeFor<IntegerIteratorDefinition>, socket: keyof IntegerIteratorDefinition["outputs"], context: Resolver.Context): DataTypes.AnyEval | null => {
    ...
    const value = Math.round(IterationPrefab.sampleStopsWith(active, position, (a, b, t) => a + t * (b - a)));
...
const onConnect = (node: NodeDefinitions.BuiltNodeOf<"integerIterator", IntegerIteratorDefinition>, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
