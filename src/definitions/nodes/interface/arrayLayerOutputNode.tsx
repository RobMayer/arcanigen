...
const onCreate = (node: NodeDefinitions.BuiltNodeOf<"arrayLayerOutput", ArrayLayerOutputDefinition>, graphId: string, ctx: NodeTypes.MethodContext): void => {
    addInterface(ctx, graphId, node.id, "out");
};

const onDelete = (node: NodeDefinitions.BuiltNodeOf<"arrayLayerOutput", ArrayLayerOutputDefinition>, graphId: string, ctx: NodeTypes.MethodContext): void => {
...
export const ArrayLayerOutputType: NodeTypes.Type<"arrayLayerOutput", ArrayLayerOutputDefinition> = {
    ...
    defaultLabel: "Output",
    iconNode: <Icon shape={NODE_ICONS.layers} color={"var(--icon-flavour)"} cutout={"scoop"} layer={ICONS.ArrowFrom.RightArc} layerColor="#fff" />,
    category: "Outputs",
    evaluate,
    Controls,
    dependsOn,
