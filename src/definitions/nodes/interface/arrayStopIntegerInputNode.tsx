...
export type ArrayStopIntegerInputDefinition = {
    inputs: never;
    outputs: {
...
const create = (_input: Partial<NodeDefinitions.PayloadTypeOf<ArrayStopIntegerInputDefinition>>, id: string = nanoid()): NodeDefinitions.BuiltNodeOf<"arrayStopIntegerInput", ArrayStopIntegerInputDefinition> => {
    return {
        id,
        in: {},
        out: {
...
const contributesTo = (_node: NodeDefinitions.NodeFor<ArrayStopIntegerInputDefinition>, _inSocket: keyof ArrayStopIntegerInputDefinition["inputs"], _deps: AllDeps): (keyof ArrayStopIntegerInputDefinition["outputs"])[] => {
    return [];
};
...
const evaluate = (node: NodeDefinitions.NodeFor<ArrayStopIntegerInputDefinition>, socket: "output", context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "output") {
        const providedInput = context.getInput?.<"array<stop<integer>>">(node.id);
...
const SOCKETTYPES_OUT: { [key in keyof Required<ArrayStopIntegerInputDefinition["outputs"]>]: SocketTypes.SocketRule } = {
    output: { types: ["array<stop<integer>>"], mode: "and" },
};

const getSocketType = (_node: NodeDefinitions.NodeFor<ArrayStopIntegerInputDefinition>, socketId: string, _side: "in" | "out"): SocketTypes.SocketRule => {
