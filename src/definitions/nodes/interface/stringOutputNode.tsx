...
const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<StringOutputDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    ...
    return (
        <TypicalNode node={node} methods={methods}>
            <SocketIn node={node} socketId={"input"} type={"string"}>
...
const onCreate = (node: NodeDefinitions.BuiltNodeOf<"stringOutput", StringOutputDefinition>, state: NodeTypes.HookState, graphId: string): NodeTypes.HookState => {
    return addInterface(state, graphId, node.id, "out");
};

const onDelete = (node: NodeDefinitions.BuiltNodeOf<"stringOutput", StringOutputDefinition>, state: NodeTypes.HookState, graphId: string): NodeTypes.HookState => {
...
export const StringOutputType: NodeTypes.Type<"stringOutput", StringOutputDefinition> = {
    ...
    defaultLabel: "Output",
    iconNode: <Icon shape={NODE_ICONS.textShape.Item} color={"var(--icon-flavour)"} cutout={"scoop"} layer={ICONS.ArrowFrom.RightArc} layerColor="#fff" />,
    iconCard: <Icon shape={NODE_ICONS.textShape.Card} color={"var(--icon-flavour)"} cutout={"scoop"} layer={ICONS.ArrowFrom.RightArc} layerColor="#fff" />,
    category: "Outputs",
    evaluate,
    Controls,
    dependsOn,
