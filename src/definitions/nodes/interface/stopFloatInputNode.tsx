...
export type StopFloatInputDefinition = {
    inputs: never;
    outputs: {
...
const contributesTo = (_node: NodeDefinitions.NodeFor<StopFloatInputDefinition>, _inSocket: keyof StopFloatInputDefinition["inputs"], _deps: AllDeps): (keyof StopFloatInputDefinition["outputs"])[] => {
    return [];
};
...
const evaluate = (node: NodeDefinitions.NodeFor<StopFloatInputDefinition>, socket: "output", context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "output") {
        const providedInput = context.getInput?.<"stop<float>">(node.id);
...
const SOCKETTYPES_OUT: { [key in keyof Required<StopFloatInputDefinition["outputs"]>]: SocketTypes.SocketRule } = {
    output: { types: ["stop<float>"], mode: "and" },
};

const getSocketType = (_node: NodeDefinitions.NodeFor<StopFloatInputDefinition>, socketId: string, _side: "in" | "out"): SocketTypes.SocketRule => {
