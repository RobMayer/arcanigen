...
export type StopColorOutputDefinition = {
    inputs: {
        input: DataTypes.Use<"stop<color>">;
    };
    outputs: never;
    payload: {
...
const dependsOn = (_node: NodeDefinitions.NodeFor<StopColorOutputDefinition>, _outSocket: keyof StopColorOutputDefinition["outputs"], _deps: AllDeps): (keyof StopColorOutputDefinition["inputs"])[] => {
    return [];
};

const contributesTo = (_node: NodeDefinitions.NodeFor<StopColorOutputDefinition>, _inSocket: keyof StopColorOutputDefinition["inputs"], _deps: AllDeps): (keyof StopColorOutputDefinition["outputs"])[] => {
    return [];
};
...
const SOCKETTYPES_IN: { [key in keyof Required<StopColorOutputDefinition["inputs"]>]: SocketTypes.SocketRule } = {
    input: { types: ["stop<color>"], mode: "or" },
};

const getSocketType = (_node: NodeDefinitions.NodeFor<StopColorOutputDefinition>, socketId: string, _side: "in" | "out"): SocketTypes.SocketRule => {
