...
const evaluate = (node: NodeDefinitions.NodeFor<CosDefinition>, socket: "output", context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "output") {
        ...
        return { kind: "float", data: `${Math.cos(radians)}` as `${number}` };
    }
    return null;
};

const getSocketType = (_node: NodeDefinitions.NodeFor<CosDefinition>, socketId: string, _side: "in" | "out", _ctx: NodeTypes.MethodContext): SocketTypes.SocketRule => {
    switch (socketId) {
        case "input":
            return TRIG_IN;
        case "output":
