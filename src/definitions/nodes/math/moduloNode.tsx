...
// ESCAPE HATCH: modulo is asymmetric (float%length = invalid; length%length = length).
...
// Modulo output kind for a given (dividend, divisor) kind pair. Returns null for invalid combos.
// same-dim % same-dim -> same-dim (remainder preserves units); dim % dimensionless -> dim; reverse -> invalid.
const moduloOutputKind = (a: string, b: string): string | null => {
    const dimA = isDim(a);
    const dimB = isDim(b);
...
const resolveModulo = (node: ModuloNode, graphId: string, ctx: NodeTypes.MethodContext): { in: Record<string, SocketTypes.Term>; out: Record<string, SocketTypes.Term> } => {
    ...
    for (const a of aContrib) {
        for (const b of bContrib) {
            const out = moduloOutputKind(a, b);
...
type AnyModuloNode = NodeDefinitions.NodeFor<ModuloDefinition>;

const recompute = (node: AnyModuloNode, graphId: string, ctx: NodeTypes.MethodContext): void => {
    const current = ctx.getNode(graphId, node.id) as ModuloNode | undefined;
    if (!current) return;
    commitSocketTypes(current, graphId, ctx, resolveModulo(current, graphId, ctx));
};

const onConnect = (node: AnyModuloNode, _linkId: string, _direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => recompute(node, graphId, ctx);
const onDisconnect = (node: AnyModuloNode, _link: { fromNode: string; fromSocket: string; toNode: string; toSocket: string }, _direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void =>
    recompute(node, graphId, ctx);
const onRefreshRequest = (node: AnyModuloNode, _socketId: string, _side: "in" | "out", _reason: NodeTypes.RefreshReason, graphId: string, ctx: NodeTypes.MethodContext): void => recompute(node, graphId, ctx);
const onPayloadChange = (node: AnyModuloNode, _prev: PayloadType, graphId: string, ctx: NodeTypes.MethodContext): void => recompute(node, graphId, ctx);

const getSocketType = (node: AnyModuloNode, socketId: string, side: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): SocketTypes.Term => {
    ...
    switch (socketId) {
        ...
        case "output": {
            const outKind = moduloOutputKind(NumericKind.kindOf(node.payload.a), NumericKind.kindOf(node.payload.b));
