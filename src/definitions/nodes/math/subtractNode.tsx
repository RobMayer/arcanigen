...
type SubtractNode = NodeDefinitions.BuiltNodeOf<"subtract", SubtractDefinition>;

const create = (_input: Partial<NodeDefinitions.PayloadTypeOf<SubtractDefinition>>, id: string = nanoid()): SubtractNode => {
    return {
        id,
        in: {
            a: null,
...
const queryDownstreamTypes = (node: SubtractNode, graphId: string, ctx: NodeTypes.MethodContext): SocketTypes.SocketRule | null => {
...
const setPayload = (nodeId: string, updates: Partial<SubtractDefinition["payload"]>, graphId: string, ctx: NodeTypes.MethodContext): void => {
...
const onConnect = (node: SubtractNode, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    const link = ctx.getLink(graphId, linkId);
    if (!link) return;
    ...
    if (direction === "in") {
        ...
        const currentNode = ctx.getNode(graphId, node.id) as SubtractNode | undefined;
...
const onDisconnect = (node: SubtractNode, link: { fromNode: string; fromSocket: string; toNode: string; toSocket: string }, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
        const socket = link.toSocket as "a" | "b";
        const otherSocket = socket === "a" ? "b" : "a";
        const payloadKey = socket === "a" ? "connectedTypeA" : "connectedTypeB";
...
const onRefreshRequest = (node: SubtractNode, socketId: string, side: "in" | "out", reason: NodeTypes.RefreshReason, graphId: string, ctx: NodeTypes.MethodContext): void => {
    const currentNode = ctx.getNode(graphId, node.id) as SubtractNode | undefined;
    if (!currentNode) return;
...
const getSocketType = (node: NodeDefinitions.NodeFor<SubtractDefinition>, socketId: string, _side: "in" | "out", _ctx: NodeTypes.MethodContext): SocketTypes.SocketRule => {
