...
const create = (input: Partial<NodeDefinitions.PayloadTypeOf<DivideDefinition>>, id: string = nanoid()): DivideNode => {
    return {
        id,
        in: {
            a: null,
...
const queryDownstreamTypes = (node: DivideNode, graphId: string, ctx: NodeTypes.MethodContext): SocketTypes.SocketRule | null => {
...
const setPayload = (nodeId: string, updates: Partial<DivideDefinition["payload"]>, graphId: string, ctx: NodeTypes.MethodContext): void => {
...
const onConnect = (node: DivideNode, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    const link = ctx.getLink(graphId, linkId);
    if (!link) return;
    ...
    if (direction === "in") {
        ...
        const currentNode = ctx.getNode(graphId, node.id) as DivideNode | undefined;
...
const onDisconnect = (node: DivideNode, link: { fromNode: string; fromSocket: string; toNode: string; toSocket: string }, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
        const socket = link.toSocket as "a" | "b";
        const otherSocket = socket === "a" ? "b" : "a";
        const payloadKey = socket === "a" ? "connectedTypeA" : "connectedTypeB";
...
const onRefreshRequest = (node: DivideNode, socketId: string, side: "in" | "out", reason: NodeTypes.RefreshReason, graphId: string, ctx: NodeTypes.MethodContext): void => {
    const currentNode = ctx.getNode(graphId, node.id) as DivideNode | undefined;
    if (!currentNode) return;
...
const getSocketType = (node: NodeDefinitions.NodeFor<DivideDefinition>, socketId: string, _side: "in" | "out", _ctx: NodeTypes.MethodContext): SocketTypes.SocketRule => {
    const { connectedTypeA, connectedTypeB, resolvedInTypes } = node.payload;
