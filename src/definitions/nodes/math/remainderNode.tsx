...
const create = (input: Partial<NodeDefinitions.PayloadTypeOf<RemainderDefinition>>, id: string = nanoid()): RemainderNode => {
    return {
        id,
        in: {
            a: null,
...
const onDisconnect = (node: RemainderNode, link: { fromNode: string; fromSocket: string; toNode: string; toSocket: string }, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
        const socket = link.toSocket as "a" | "b";
        const otherSocket = socket === "a" ? "b" : "a";
        const payloadKey = socket === "a" ? "connectedTypeA" : "connectedTypeB";
