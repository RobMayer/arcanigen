...
export type MultiplyDefinition = {
    inputs: {
...
type MultiplyNode = NodeDefinitions.BuiltNodeOf<"multiply", MultiplyDefinition>;

const create = (_input: Partial<NodeDefinitions.PayloadTypeOf<MultiplyDefinition>>, id: string = nanoid()): MultiplyNode => {
    return {
        id,
        in: {
            a: null,
...
const queryDownstreamTypes = (node: MultiplyNode, graphId: string, ctx: NodeTypes.MethodContext): SocketTypes.SocketRule | null => {
...
const setPayload = (nodeId: string, updates: Partial<MultiplyDefinition["payload"]>, graphId: string, ctx: NodeTypes.MethodContext): void => {
...
const onConnect = (node: MultiplyNode, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    const link = ctx.getLink(graphId, linkId);
    if (!link) return;
    ...
    if (direction === "in") {
        ...
        const currentNode = ctx.getNode(graphId, node.id) as MultiplyNode | undefined;
...
const onRefreshRequest = (node: MultiplyNode, socketId: string, side: "in" | "out", reason: NodeTypes.RefreshReason, graphId: string, ctx: NodeTypes.MethodContext): void => {
    const currentNode = ctx.getNode(graphId, node.id) as MultiplyNode | undefined;
    if (!currentNode) return;
...
const getSocketType = (node: NodeDefinitions.NodeFor<MultiplyDefinition>, socketId: string, _side: "in" | "out", _ctx: NodeTypes.MethodContext): SocketTypes.SocketRule => {
