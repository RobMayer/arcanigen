...
const create = (input: Partial<NodeDefinitions.PayloadTypeOf<AbsDefinition>>, id: string = nanoid()): AbsNode => {
    return {
        id,
        in: { input: null },
        out: { output: [] },
...
const queryDownstreamTypes = (node: AbsNode, graphId: string, ctx: NodeTypes.MethodContext): SocketTypes.SocketRule | null => {
...
const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<AbsDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    const handleUpdate = useCallback(
        (v: Partial<NodeDefinitions.PayloadTypeOf<AbsDefinition>>) => {
            methods.update(v);
        },
        [methods],
    );
...
const evaluate = (node: NodeDefinitions.NodeFor<AbsDefinition>, socket: "output", context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "output") {
        ...
        return wrapResult(Math.abs(value), kind, unit);
    }
    return null;
};

const setPayload = (nodeId: string, updates: Partial<AbsDefinition["payload"]>, graphId: string, ctx: NodeTypes.MethodContext): void => {
...
const onConnect = (node: AbsNode, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    const link = ctx.getLink(graphId, linkId);
    if (!link) return;
    ...
    if (direction === "in") {
        ...
        const currentNode = ctx.getNode(graphId, node.id) as AbsNode | undefined;
...
const onDisconnect = (node: AbsNode, link: { fromNode: string; fromSocket: string; toNode: string; toSocket: string }, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
...
const onRefreshRequest = (node: AbsNode, socketId: string, side: "in" | "out", reason: NodeTypes.RefreshReason, graphId: string, ctx: NodeTypes.MethodContext): void => {
    const currentNode = ctx.getNode(graphId, node.id) as AbsNode | undefined;
    if (!currentNode) return;
...
const getSocketType = (node: NodeDefinitions.NodeFor<AbsDefinition>, socketId: string, _side: "in" | "out", _ctx: NodeTypes.MethodContext): SocketTypes.SocketRule => {
    const t = effectiveType(node.payload.connectedType, node.payload.resolvedInTypes);
    switch (socketId) {
        case "input":
        case "output":
...
export const AbsType: NodeTypes.Type<"abs", AbsDefinition> = {
    ...
    displayName: "Abs",
    defaultLabel: "Abs",
    iconNode: <Icon shape={NODE_ICONS.absValue.Item} color={"var(--icon-flavour)"} />,
    iconCard: <Icon shape={NODE_ICONS.absValue.Card} color={"var(--icon-flavour)"} />,
    category: "Math",
    evaluate,
    Controls,
    dependsOn,
