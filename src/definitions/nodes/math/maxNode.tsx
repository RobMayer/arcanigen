...
type MaxNode = NodeDefinitions.BuiltNodeOf<"max", MaxDefinition>;

const create = (_input: Partial<NodeDefinitions.PayloadTypeOf<MaxDefinition>>, id: string = nanoid()): MaxNode => {
    return {
        id,
        in: {
            a: null,
...
const queryDownstreamTypes = (node: MaxNode, graphId: string, ctx: NodeTypes.MethodContext): SocketTypes.SocketRule | null => {
...
const dependsOn = (_node: NodeDefinitions.NodeFor<MaxDefinition>, outSocket: "output", _deps: AllDeps): (keyof MaxDefinition["inputs"])[] => {
    if (outSocket === "output") return ["a", "b"];
    return [];
};

const contributesTo = (_node: NodeDefinitions.NodeFor<MaxDefinition>, _inSocket: keyof MaxDefinition["inputs"], _deps: AllDeps): (keyof MaxDefinition["outputs"])[] => {
    return ["output"];
};
...
const setPayload = (nodeId: string, updates: Partial<MaxDefinition["payload"]>, graphId: string, ctx: NodeTypes.MethodContext): void => {
...
const onConnect = (node: MaxNode, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    const link = ctx.getLink(graphId, linkId);
    if (!link) return;
    ...
    if (direction === "in") {
        ...
        const currentNode = ctx.getNode(graphId, node.id) as MaxNode | undefined;
...
const onDisconnect = (node: MaxNode, link: { fromNode: string; fromSocket: string; toNode: string; toSocket: string }, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
        const socket = link.toSocket as "a" | "b";
        const otherSocket = socket === "a" ? "b" : "a";
        const payloadKey = socket === "a" ? "connectedTypeA" : "connectedTypeB";
...
const onRefreshRequest = (node: MaxNode, socketId: string, side: "in" | "out", reason: NodeTypes.RefreshReason, graphId: string, ctx: NodeTypes.MethodContext): void => {
    const currentNode = ctx.getNode(graphId, node.id) as MaxNode | undefined;
    if (!currentNode) return;
...
const getSocketType = (node: NodeDefinitions.NodeFor<MaxDefinition>, socketId: string, _side: "in" | "out", _ctx: NodeTypes.MethodContext): SocketTypes.SocketRule => {
    const { connectedTypeA, connectedTypeB, resolvedInTypes } = node.payload;
...
export const MaxType: NodeTypes.Type<"max", MaxDefinition> = {
    type: "max",
    displayName: "Max",
    defaultLabel: "Max",
