...
type MinNode = NodeDefinitions.BuiltNodeOf<"min", MinDefinition>;

const create = (input: Partial<NodeDefinitions.PayloadTypeOf<MinDefinition>>, id: string = nanoid()): MinNode => {
    return {
        id,
        in: {
            a: null,
...
const queryDownstreamTypes = (node: MinNode, graphId: string, ctx: NodeTypes.MethodContext): SocketTypes.SocketRule | null => {
...
const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<MinDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    const handleUpdate = useCallback(
        (v: Partial<NodeDefinitions.PayloadTypeOf<MinDefinition>>) => {
            methods.update(v);
        },
        [methods],
    );
...
const setPayload = (nodeId: string, updates: Partial<MinDefinition["payload"]>, graphId: string, ctx: NodeTypes.MethodContext): void => {
...
const onConnect = (node: MinNode, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    const link = ctx.getLink(graphId, linkId);
    if (!link) return;
    ...
    if (direction === "in") {
        ...
        const currentNode = ctx.getNode(graphId, node.id) as MinNode | undefined;
...
const onRefreshRequest = (node: MinNode, socketId: string, side: "in" | "out", reason: NodeTypes.RefreshReason, graphId: string, ctx: NodeTypes.MethodContext): void => {
    const currentNode = ctx.getNode(graphId, node.id) as MinNode | undefined;
    if (!currentNode) return;
...
const getSocketType = (node: NodeDefinitions.NodeFor<MinDefinition>, socketId: string, _side: "in" | "out", _ctx: NodeTypes.MethodContext): SocketTypes.SocketRule => {
    const { connectedTypeA, connectedTypeB, resolvedInTypes } = node.payload;
