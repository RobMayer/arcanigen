...
const def = signature({
    in: { stops: $.arrayOf("stop:integer"), "stop_*": "stop:integer" },
    out: { output: $.arrayOf("stop:integer"), stopCount: "integer" },
});

export type IntegerStopArrayDefinition = SignatureBuilder.DefinitionFrom<
    typeof def,
    {
        label: string;
        stops: StopEntryData[];
...
const create = (input: Partial<NodeDefinitions.PayloadTypeOf<IntegerStopArrayDefinition>>, id: string = nanoid()): NodeDefinitions.BuiltNodeOf<"integerStopArray", IntegerStopArrayDefinition> => {
    ...
    return {
        ...
        type: "integerStopArray",
    };
};

const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<IntegerStopArrayDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    const { alterNode, removeLinks } = Project.useMethods();

    const handleUpdate = useCallback(
        (v: Partial<NodeDefinitions.PayloadTypeOf<IntegerStopArrayDefinition>>) => {
            methods.update(v);
        },
        [methods],
    );
    ...
    const handleAddStop = useCallback(() => {
        ...
        alterNode(node.id, (n) => ({
            ...
            payload: {
                ...n.payload,
                stops: [...(n.payload as IntegerStopArrayDefinition["payload"]).stops, { socket, value: "5", position: "50", enabled: true }],
            },
        }));
    }, [alterNode, node.id]);
        ...
        (socket: string) => {
            ...
            alterNode(node.id, (n) => {
                ...
                return {
                    ...
                    payload: {
                        ...n.payload,
                        stops: (n.payload as IntegerStopArrayDefinition["payload"]).stops.filter((s) => s.socket !== socket),
                    },
                };
            });
        },
...
const StopEntry = ({
    ...
    entry: StopEntryData;
    node: NodeDefinitions.NodeFor<IntegerStopArrayDefinition>;
    index: number;
...
// Resolve the effective stops: the supersocket (an array<stop:integer>) overrides everything;
// otherwise fold each per-stop socket (a connected stop:integer) over its inline payload.
const resolveStops = (node: NodeDefinitions.NodeFor<IntegerStopArrayDefinition>, context: Resolver.Context): { value: number; position: number; enabled: boolean }[] => {
    const supersocketEval = context.resolve<DataTypes.ArrayOf<DataTypes.StopInteger>>(node.id, "stops");
    if (supersocketEval) {
    ...
    for (const entry of node.payload.stops) {
        const connected = context.resolve<DataTypes.StopInteger>(node.id, entry.socket);
        if (connected) {
            resolved.push({
...
const dependsOn = (
    node: NodeDefinitions.NodeFor<IntegerStopArrayDefinition>,
    outSocket: keyof IntegerStopArrayDefinition["outputs"],
    _deps: AllDeps,
): (keyof IntegerStopArrayDefinition["inputs"])[] => {
...
const contributesTo = (
    _node: NodeDefinitions.NodeFor<IntegerStopArrayDefinition>,
    _inSocket: keyof IntegerStopArrayDefinition["inputs"],
    _deps: AllDeps,
): (keyof IntegerStopArrayDefinition["outputs"])[] => {
    return ["output", "stopCount"];
};

const evaluate = (node: NodeDefinitions.NodeFor<IntegerStopArrayDefinition>, socket: keyof IntegerStopArrayDefinition["outputs"], context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "stopCount") {
    ...
    if (socket === "output") {
        ...
        return { kind: "array<stop:integer>", data: stops.map((s) => ({ value: Math.round(s.value), position: s.position, enabled: s.enabled })) };
    }
    return null;
};

// Supersocket override: connecting the whole `array<stop:integer>` clears the now-hidden element family,
// then hands off to the engine (a no-op for this var-free def, kept for uniform wiring).
const onConnect = (node: NodeDefinitions.BuiltNodeOf<"integerStopArray", IntegerStopArrayDefinition>, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
...
export const IntegerStopArrayNodeType: NodeTypes.Type<"integerStopArray", IntegerStopArrayDefinition> = {
    type: "integerStopArray",
    displayName: "Integer Stop Array",
    defaultLabel: "Integer Stop Array",
