...
type StringEntryData = { socket: string; value: string };

const def = signature({
    in: { values: $.arrayOf("string"), "value_*": "string" },
    out: { output: $.arrayOf("string"), count: "integer" },
});

export type StringArrayDefinition = SignatureBuilder.DefinitionFrom<
    typeof def,
    {
        label: string;
        values: StringEntryData[];
    }
>;

const newEntry = (socket: `value_${string}`, value: string): StringEntryData => ({ socket, value });

const create = (input: Partial<NodeDefinitions.PayloadTypeOf<StringArrayDefinition>>, id: string = nanoid()): NodeDefinitions.BuiltNodeOf<"stringArray", StringArrayDefinition> => {
...
const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<StringArrayDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    const { alterNode, removeLinks } = Project.useMethods();

    const handleUpdate = useCallback(
        (v: Partial<NodeDefinitions.PayloadTypeOf<StringArrayDefinition>>) => {
            methods.update(v);
        },
        [methods],
    );
    ...
    const handleAddValue = useCallback(() => {
        ...
        alterNode(node.id, (n) => ({
            ...
            payload: {
                ...n.payload,
                values: [...(n.payload as StringArrayDefinition["payload"]).values, entry],
            },
        }));
    }, [alterNode, node.id]);
        ...
        (socket: string) => {
            ...
            alterNode(node.id, (n) => {
                ...
                return {
                    ...
                    payload: {
                        ...n.payload,
                        values: (n.payload as StringArrayDefinition["payload"]).values.filter((p) => p.socket !== socket),
                    },
                };
            });
        },
    ...
    const handleValueUpdate = useCallback(
        (socket: string, update: Partial<StringEntryData>) => {
            handleUpdate({
...
const VALUE_MIME = "application/x-string-array-socket";
...
const ValueEntry = ({
    ...
    entry: StringEntryData;
    node: NodeDefinitions.NodeFor<StringArrayDefinition>;
    index: number;
    handleValueUpdate: (socket: string, update: Partial<StringEntryData>) => void;
    ...
    return (
        <ValueEntryWrapper ref={ref} data-state={dropSide ? `drop-${dropSide}` : undefined} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onDragEnd={handleDragEnd}>
            <SocketIn node={node} socketId={entry.socket as `value_${string}`}>
                <ValueRow>
                    <TextInput className={"valueField"} value={entry.value} onCommit={(value) => handleValueUpdate(entry.socket, { value })} disabled={connected} />
...
const resolveValues = (node: NodeDefinitions.NodeFor<StringArrayDefinition>, context: Resolver.Context): string[] => {
    const supersocketEval = context.resolve<DataTypes.ArrayOf<DataTypes.String>>(node.id, "values");
    if (supersocketEval) {
    ...
    for (const entry of node.payload.values) {
        const connected = context.resolve<DataTypes.String>(node.id, entry.socket);
        resolved.push(connected ? connected.data : entry.value);
    }
    return resolved;
};

const dependsOn = (node: NodeDefinitions.NodeFor<StringArrayDefinition>, outSocket: keyof StringArrayDefinition["outputs"], _deps: AllDeps): (keyof StringArrayDefinition["inputs"])[] => {
...
const contributesTo = (_node: NodeDefinitions.NodeFor<StringArrayDefinition>, _inSocket: keyof StringArrayDefinition["inputs"], _deps: AllDeps): (keyof StringArrayDefinition["outputs"])[] => {
    return ["output", "count"];
};

const evaluate = (node: NodeDefinitions.NodeFor<StringArrayDefinition>, socket: keyof StringArrayDefinition["outputs"], context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "count") {
        return { kind: "integer", data: `${resolveValues(node, context).length}` };
    }
    if (socket === "output") {
        return { kind: "array<string>", data: resolveValues(node, context) };
    }
    return null;
};

const onConnect = (node: NodeDefinitions.BuiltNodeOf<"stringArray", StringArrayDefinition>, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
...
export const StringArrayNodeType: NodeTypes.Type<"stringArray", StringArrayDefinition> = {
    type: "stringArray",
    displayName: "String Array",
    defaultLabel: "String Array",
    iconNode: <NodeIcon shape={NODE_ICONS.text} modifierIcon={NODE_ICONS.modifiers.arrayOf} />,
    flavour: "danger",
    category: "Values",
    create,
    dependsOn,
