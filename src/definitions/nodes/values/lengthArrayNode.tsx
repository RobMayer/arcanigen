...
type LengthEntryData = { socket: string; value: EmptyOr<Length.Type> };

const def = signature({
    in: { values: $.arrayOf("length"), "value_*": "length" },
    out: { output: $.arrayOf("length"), count: "integer" },
});

export type LengthArrayDefinition = SignatureBuilder.DefinitionFrom<
    typeof def,
    {
        label: string;
        values: LengthEntryData[];
    }
>;

const newEntry = (socket: `value_${string}`, value: EmptyOr<Length.Type>): LengthEntryData => ({ socket, value });

const create = (input: Partial<NodeDefinitions.PayloadTypeOf<LengthArrayDefinition>>, id: string = nanoid()): NodeDefinitions.BuiltNodeOf<"lengthArray", LengthArrayDefinition> => {
    ...
    return {
        ...
        payload: {
            label: "",
            values: input.values ?? [newEntry(s0, "0px"), newEntry(s1, "100px")],
        },
        type: "lengthArray",
    };
};

const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<LengthArrayDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    const { alterNode, removeLinks } = Project.useMethods();

    const handleUpdate = useCallback(
        (v: Partial<NodeDefinitions.PayloadTypeOf<LengthArrayDefinition>>) => {
            methods.update(v);
        },
        [methods],
    );
    ...
    const handleAddValue = useCallback(() => {
        const entry = newEntry(`value_${nanoid()}`, "0px");
        alterNode(node.id, (n) => ({
            ...n,
            in: { ...n.in, [entry.socket]: null },
            payload: {
                ...n.payload,
                values: [...(n.payload as LengthArrayDefinition["payload"]).values, entry],
            },
        }));
    }, [alterNode, node.id]);
        ...
        (socket: string) => {
            ...
            alterNode(node.id, (n) => {
                ...
                return {
                    ...
                    payload: {
                        ...n.payload,
                        values: (n.payload as LengthArrayDefinition["payload"]).values.filter((p) => p.socket !== socket),
                    },
                };
            });
        },
    ...
    const handleValueUpdate = useCallback(
        (socket: string, update: Partial<LengthEntryData>) => {
            handleUpdate({
...
const VALUE_MIME = "application/x-length-array-socket";
...
const ValueEntry = ({
    ...
    entry: LengthEntryData;
    node: NodeDefinitions.NodeFor<LengthArrayDefinition>;
    index: number;
    handleValueUpdate: (socket: string, update: Partial<LengthEntryData>) => void;
    ...
    return (
        <ValueEntryWrapper ref={ref} data-state={dropSide ? `drop-${dropSide}` : undefined} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onDragEnd={handleDragEnd}>
            <SocketIn node={node} socketId={entry.socket as `value_${string}`}>
                <ValueRow>
                    <LengthInput className={"valueField"} value={entry.value} onCommit={(value) => handleValueUpdate(entry.socket, { value })} disabled={connected} required />
...
// Resolve the effective values: the supersocket (an array<length>) overrides everything; otherwise
// fold each per-value socket (a connected length) over its inline authoring field.
const resolveValues = (node: NodeDefinitions.NodeFor<LengthArrayDefinition>, context: Resolver.Context): EmptyOr<Length.Type>[] => {
    const supersocketEval = context.resolve<DataTypes.ArrayOf<DataTypes.Length>>(node.id, "values");
    if (supersocketEval) {
        return supersocketEval.data;
    }

    const resolved: EmptyOr<Length.Type>[] = [];
    for (const entry of node.payload.values) {
        const connected = context.resolve<DataTypes.Length>(node.id, entry.socket);
        resolved.push(connected ? connected.data : entry.value);
    }
    return resolved;
};

const dependsOn = (node: NodeDefinitions.NodeFor<LengthArrayDefinition>, outSocket: keyof LengthArrayDefinition["outputs"], _deps: AllDeps): (keyof LengthArrayDefinition["inputs"])[] => {
...
const contributesTo = (_node: NodeDefinitions.NodeFor<LengthArrayDefinition>, _inSocket: keyof LengthArrayDefinition["inputs"], _deps: AllDeps): (keyof LengthArrayDefinition["outputs"])[] => {
    return ["output", "count"];
};

const evaluate = (node: NodeDefinitions.NodeFor<LengthArrayDefinition>, socket: keyof LengthArrayDefinition["outputs"], context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "count") {
        return { kind: "integer", data: `${resolveValues(node, context).length}` };
    }
    if (socket === "output") {
        return { kind: "array<length>", data: resolveValues(node, context) };
    }
    return null;
};

// Supersocket override: connecting the whole `array<length>` clears the now-hidden element family,
// then hands off to the engine (a no-op for this var-free def, kept for uniform wiring).
const onConnect = (node: NodeDefinitions.BuiltNodeOf<"lengthArray", LengthArrayDefinition>, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
...
export const LengthArrayNodeType: NodeTypes.Type<"lengthArray", LengthArrayDefinition> = {
    type: "lengthArray",
