...
type IntegerEntryData = { socket: string; value: EmptyOr<NumericString.Type> };

const def = signature({
    in: { values: $.arrayOf("integer"), "value_*": "integer" },
    out: { output: $.arrayOf("integer"), count: "integer" },
});

export type IntegerArrayDefinition = SignatureBuilder.DefinitionFrom<
    typeof def,
    {
        label: string;
        values: IntegerEntryData[];
    }
>;

const newEntry = (socket: `value_${string}`, value: EmptyOr<NumericString.Type>): IntegerEntryData => ({ socket, value });

const create = (input: Partial<NodeDefinitions.PayloadTypeOf<IntegerArrayDefinition>>, id: string = nanoid()): NodeDefinitions.BuiltNodeOf<"integerArray", IntegerArrayDefinition> => {
...
const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<IntegerArrayDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    const { alterNode, removeLinks } = Project.useMethods();

    const handleUpdate = useCallback(
        (v: Partial<NodeDefinitions.PayloadTypeOf<IntegerArrayDefinition>>) => {
            methods.update(v);
        },
        [methods],
    );
    ...
    const handleAddValue = useCallback(() => {
        ...
        alterNode(node.id, (n) => ({
            ...
            payload: {
                ...n.payload,
                values: [...(n.payload as IntegerArrayDefinition["payload"]).values, entry],
            },
        }));
    }, [alterNode, node.id]);
        ...
        (socket: string) => {
            ...
            alterNode(node.id, (n) => {
                ...
                return {
                    ...
                    payload: {
                        ...n.payload,
                        values: (n.payload as IntegerArrayDefinition["payload"]).values.filter((p) => p.socket !== socket),
                    },
                };
            });
        },
    ...
    const handleValueUpdate = useCallback(
        (socket: string, update: Partial<IntegerEntryData>) => {
            handleUpdate({
...
const VALUE_MIME = "application/x-integer-array-socket";
...
const ValueEntry = ({
    ...
    entry: IntegerEntryData;
    node: NodeDefinitions.NodeFor<IntegerArrayDefinition>;
    index: number;
    handleValueUpdate: (socket: string, update: Partial<IntegerEntryData>) => void;
    ...
    return (
        <ValueEntryWrapper ref={ref} data-state={dropSide ? `drop-${dropSide}` : undefined} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onDragEnd={handleDragEnd}>
            <SocketIn node={node} socketId={entry.socket as `value_${string}`}>
                <ValueRow>
                    <IntegerInput className={"valueField"} value={entry.value} onCommit={(value) => handleValueUpdate(entry.socket, { value })} disabled={connected} />
...
// Resolve the effective values: the supersocket (an array<integer>) overrides everything; otherwise
// fold each per-value socket (a connected integer) over its inline authoring field.
const resolveValues = (node: NodeDefinitions.NodeFor<IntegerArrayDefinition>, context: Resolver.Context): EmptyOr<NumericString.Type>[] => {
    const supersocketEval = context.resolve<DataTypes.ArrayOf<DataTypes.Integer>>(node.id, "values");
    if (supersocketEval) {
    ...
    for (const entry of node.payload.values) {
        const connected = context.resolve<DataTypes.Integer>(node.id, entry.socket);
        resolved.push(connected ? connected.data : entry.value);
    }
    return resolved;
};

const dependsOn = (node: NodeDefinitions.NodeFor<IntegerArrayDefinition>, outSocket: keyof IntegerArrayDefinition["outputs"], _deps: AllDeps): (keyof IntegerArrayDefinition["inputs"])[] => {
...
const contributesTo = (_node: NodeDefinitions.NodeFor<IntegerArrayDefinition>, _inSocket: keyof IntegerArrayDefinition["inputs"], _deps: AllDeps): (keyof IntegerArrayDefinition["outputs"])[] => {
    return ["output", "count"];
};

const evaluate = (node: NodeDefinitions.NodeFor<IntegerArrayDefinition>, socket: keyof IntegerArrayDefinition["outputs"], context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "count") {
        return { kind: "integer", data: `${resolveValues(node, context).length}` };
    }
    if (socket === "output") {
        return { kind: "array<integer>", data: resolveValues(node, context) };
    }
    return null;
};

// Supersocket override: connecting the whole `array<integer>` clears the now-hidden element family,
// then hands off to the engine (a no-op for this var-free def, kept for uniform wiring).
const onConnect = (node: NodeDefinitions.BuiltNodeOf<"integerArray", IntegerArrayDefinition>, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
...
export const IntegerArrayNodeType: NodeTypes.Type<"integerArray", IntegerArrayDefinition> = {
    type: "integerArray",
