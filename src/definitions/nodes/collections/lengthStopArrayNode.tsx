...
const create = (input: Partial<NodeDefinitions.PayloadTypeOf<LengthStopArrayDefinition>>, id: string = nanoid()): NodeDefinitions.BuiltNodeOf<"lengthStopArray", LengthStopArrayDefinition> => {
    ...
    return {
        ...
        type: "lengthStopArray",
    };
};

const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<LengthStopArrayDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    const { alterNode, removeLinks } = Project.useMethods();

    const handleUpdate = useCallback(
        (v: Partial<NodeDefinitions.PayloadTypeOf<LengthStopArrayDefinition>>) => {
            methods.update(v);
        },
        [methods],
    );
    ...
    const handleAddStop = useCallback(() => {
        ...
        alterNode(node.id, (n) => ({
            ...
            payload: {
                ...n.payload,
                stops: [...(n.payload as LengthStopArrayDefinition["payload"]).stops, { socket, value: "50px", position: "50", enabled: true }],
            },
        }));
    }, [alterNode, node.id]);
        ...
        (socket: string) => {
            ...
            alterNode(node.id, (n) => {
                ...
                return {
                    ...
                    payload: {
                        ...n.payload,
                        stops: (n.payload as LengthStopArrayDefinition["payload"]).stops.filter((s) => s.socket !== socket),
                    },
                };
            });
        },
...
const StopEntry = ({
    ...
    entry: StopEntryData;
    node: NodeDefinitions.NodeFor<LengthStopArrayDefinition>;
    index: number;
...
const dependsOn = (node: NodeDefinitions.NodeFor<LengthStopArrayDefinition>, outSocket: keyof LengthStopArrayDefinition["outputs"], _deps: AllDeps): (keyof LengthStopArrayDefinition["inputs"])[] => {
...
const contributesTo = (
    _node: NodeDefinitions.NodeFor<LengthStopArrayDefinition>,
    _inSocket: keyof LengthStopArrayDefinition["inputs"],
    _deps: AllDeps,
): (keyof LengthStopArrayDefinition["outputs"])[] => {
    return ["output", "stopCount"];
};

const evaluate = (node: NodeDefinitions.NodeFor<LengthStopArrayDefinition>, socket: keyof LengthStopArrayDefinition["outputs"], context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "stopCount") {
...
export const LengthStopArrayNodeType: NodeTypes.Type<"lengthStopArray", LengthStopArrayDefinition> = {
    type: "lengthStopArray",
    displayName: "Length Stop Array",
    defaultLabel: "Length Stop Array",
    iconNode: <Icon shape={NODE_ICONS.length} color={"var(--icon-flavour)"} cutout={"scoop"} layer={NODE_ICONS.modifiers.arrayOfStopOf} layerColor="#fff" />,
