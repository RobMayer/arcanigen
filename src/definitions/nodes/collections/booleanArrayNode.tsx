...
type BooleanEntryData = { socket: string; value: boolean };

const def = signature({
    in: { values: $.arrayOf("boolean"), "value_*": "boolean" },
    out: { output: $.arrayOf("boolean"), count: "integer" },
});

export type BooleanArrayDefinition = SignatureBuilder.DefinitionFrom<
    typeof def,
    {
        label: string;
        values: BooleanEntryData[];
    }
>;

const newEntry = (socket: `value_${string}`, value: boolean): BooleanEntryData => ({ socket, value });

const create = (input: Partial<NodeDefinitions.PayloadTypeOf<BooleanArrayDefinition>>, id: string = nanoid()): NodeDefinitions.BuiltNodeOf<"booleanArray", BooleanArrayDefinition> => {
    ...
    return {
        ...
        payload: {
            label: "",
            values: input.values ?? [newEntry(s0, true), newEntry(s1, false)],
        },
        type: "booleanArray",
    };
};

const Controls = ({ node, methods }: { node: NodeDefinitions.NodeFor<BooleanArrayDefinition>; methods: ReturnType<typeof Project.useNode>[1] }): ReactNode => {
    const { alterNode, removeLinks } = Project.useMethods();

    const handleUpdate = useCallback(
        (v: Partial<NodeDefinitions.PayloadTypeOf<BooleanArrayDefinition>>) => {
            methods.update(v);
        },
        [methods],
    );
    ...
    const handleAddValue = useCallback(() => {
        const entry = newEntry(`value_${nanoid()}`, false);
        alterNode(node.id, (n) => ({
            ...n,
            in: { ...n.in, [entry.socket]: null },
            payload: {
                ...n.payload,
                values: [...(n.payload as BooleanArrayDefinition["payload"]).values, entry],
            },
        }));
    }, [alterNode, node.id]);
        ...
        (socket: string) => {
            ...
            alterNode(node.id, (n) => {
                ...
                return {
                    ...
                    payload: {
                        ...n.payload,
                        values: (n.payload as BooleanArrayDefinition["payload"]).values.filter((p) => p.socket !== socket),
                    },
                };
            });
        },
    ...
    const handleValueUpdate = useCallback(
        (socket: string, update: Partial<BooleanEntryData>) => {
            handleUpdate({
...
const VALUE_MIME = "application/x-boolean-array-socket";
...
const ValueEntry = ({
    ...
    entry: BooleanEntryData;
    node: NodeDefinitions.NodeFor<BooleanArrayDefinition>;
    index: number;
    handleValueUpdate: (socket: string, update: Partial<BooleanEntryData>) => void;
    ...
    return (
        <ValueEntryWrapper ref={ref} data-state={dropSide ? `drop-${dropSide}` : undefined} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onDragEnd={handleDragEnd}>
            <SocketIn node={node} socketId={entry.socket as `value_${string}`}>
                <ValueRow>
                    <CheckBox className={"valueField"} checked={entry.value} onToggle={(value) => handleValueUpdate(entry.socket, { value })} disabled={connected}>
...
// Resolve the effective values: the supersocket (an array<boolean>) overrides everything; otherwise
// fold each per-value socket (a connected boolean) over its inline authoring field.
const resolveValues = (node: NodeDefinitions.NodeFor<BooleanArrayDefinition>, context: Resolver.Context): boolean[] => {
    const supersocketEval = context.resolve<DataTypes.ArrayOf<DataTypes.Boolean>>(node.id, "values");
    if (supersocketEval) {
    ...
    for (const entry of node.payload.values) {
        const connected = context.resolve<DataTypes.Boolean>(node.id, entry.socket);
        resolved.push(connected ? connected.data : entry.value);
    }
    return resolved;
};

const dependsOn = (node: NodeDefinitions.NodeFor<BooleanArrayDefinition>, outSocket: keyof BooleanArrayDefinition["outputs"], _deps: AllDeps): (keyof BooleanArrayDefinition["inputs"])[] => {
...
const contributesTo = (_node: NodeDefinitions.NodeFor<BooleanArrayDefinition>, _inSocket: keyof BooleanArrayDefinition["inputs"], _deps: AllDeps): (keyof BooleanArrayDefinition["outputs"])[] => {
    return ["output", "count"];
};

const evaluate = (node: NodeDefinitions.NodeFor<BooleanArrayDefinition>, socket: keyof BooleanArrayDefinition["outputs"], context: Resolver.Context): DataTypes.AnyEval | null => {
    if (socket === "count") {
        return { kind: "integer", data: `${resolveValues(node, context).length}` };
    }
    if (socket === "output") {
        return { kind: "array<boolean>", data: resolveValues(node, context) };
    }
    return null;
};

// Supersocket override: connecting the whole `array<boolean>` clears the now-hidden element family,
// then hands off to the engine (a no-op for this var-free def, kept for uniform wiring).
const onConnect = (node: NodeDefinitions.BuiltNodeOf<"booleanArray", BooleanArrayDefinition>, linkId: string, direction: "in" | "out", graphId: string, ctx: NodeTypes.MethodContext): void => {
    if (direction === "in") {
...
export const BooleanArrayNodeType: NodeTypes.Type<"booleanArray", BooleanArrayDefinition> = {
    type: "booleanArray",
