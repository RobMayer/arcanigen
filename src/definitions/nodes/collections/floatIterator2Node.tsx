        ...
        <StopEntryWrapper ref={ref} data-state={dropSide ? `drop-${dropSide}` : undefined} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onDragEnd={handleDragEnd}>
            <SocketIn node={node} socketId={entry.socket as `stop_${string}`}>
                <StopRow>
                    ...
                    <DecimalInput value={entry.value} onCommit={(value) => handleStopUpdate(entry.socket, { value })} disabled={connected} />
...
const StopRow = styled.div`
    ...
    /* Let the numeric inputs shrink rather than forcing the node wide.
       COMMON_STYLES.INPUT defaults to flex: 1 1 auto (~20ch intrinsic width);
       a small basis via this higher-specificity descendant rule overrides it.
       Scoped to type="text" so the enabled checkbox is untouched. */
    & input[type="text"] {
        /* width:0 → no max-content contribution, so the inputs never push the
           node past its 280px floor; flex grows them to fill the row at layout. */
        flex: 1 1 0;
        width: 0;
        min-width: 0;
    }
